...
export class WithdrawalService {
  ...
  static async requestWithdrawal(
    ...
    // MANDATORY FINANCIAL COMPLIANCE: User must have verified KYC
    if (user.kycStatus !== 'VERIFIED') {
      return {
        success: false,
        message: 'KYC_REQUIRED: Mandatory identity verification required before requesting withdrawals. Please complete KYC verification in your wallet.',
      };
    }

    // Verify user has verified payout methods if any configured
    const userPayoutMethods = await DatabaseService.getPayoutMethodsByUserId(userId);
    if (userPayoutMethods.length > 0) {
      const hasVerified = userPayoutMethods.some((m) => m.isVerified);
      if (!hasVerified) {
        return {
          success: false,
          message: 'VERIFIED_PAYOUT_METHOD_REQUIRED: You must have at least one verified payout method to withdraw.',
        };
      }
    }
    ...
    const userUnits = await DatabaseService.getUnits(user.id);
    ...
    const debitRes = await DatabaseService.atomicDebitBalance(user.id, amountUsdt);
    if (!debitRes.success) {
      return {
        success: false,
    ...
    const request = await DatabaseService.createWithdrawal({
      userId: user.id,
      userName: user.name,
  ...
  static async processAdminAction(
    ...
    const request = await DatabaseService.getWithdrawalById(withdrawalId);
    ...
    if (action === 'REJECT') {
      ...
      // 1. Atomically transition status to REJECTED first
      const updated = await DatabaseService.transitionWithdrawalStatus(
        withdrawalId,
        previousStatus,
        'REJECTED',
        {
      ...
      if (!updated) {
        return {
          success: false,
          message: `Illegal transition: Withdrawal ${withdrawalId} status could not be transitioned from ${previousStatus}.`,
        };
      }

      // 2. Refund balance ONCE atomically only after successful state transition
      const creditRes = await DatabaseService.atomicCreditBalance(request.userId, request.amountUsdt);
    ...
    if (action === 'COMPLETE') {
      ...
      const updated = await DatabaseService.updateWithdrawal(withdrawalId, {
        status: 'COMPLETED',
