...
export class SolarGenerationService {
  ...
  /**
   * Server-side Idempotent Daily Settlement against Supabase PostgreSQL.
   */
  static async settleDailyOperation(
    ...
    if (remaining <= 0) {
      await SupabaseDatabaseService.updateUnit(unit.id, { status: 'EXPIRED' });
      return { success: false, message: 'This solar unit has completed its working days lifecycle.' };
    }

    // Check idempotency
    const existingLogs = await SupabaseDatabaseService.getGenerationLogs(unit.userId);
    ...
    const existingLedger = (await SupabaseDatabaseService.getLedger(unit.userId)).find(
      (l) => l.referenceId === idempotencyKey
    );

    if (alreadySettledLog || existingLedger || (unit.lastOperatedDate === todayStr && !unit.isReceivable)) {
      return {
        success: true,
        alreadySettled: true,
    ...
    const user = await SupabaseDatabaseService.getUserById(unit.userId);
    if (!user) {
      return { success: false, message: 'User account not found' };
    }

    const plan = await SupabaseDatabaseService.getPlanByCode(unit.planCode);
    ...
    if (log) {
      const updatedLog = await SupabaseDatabaseService.updateGenerationLog(log.id, {
        status: 'RECEIVED',
      ...
      log = await SupabaseDatabaseService.createGenerationLog({
        unitId: unit.id,
        userId: unit.userId,
    ...
    const updatedUnit = await SupabaseDatabaseService.updateUnit(unit.id, {
  ...
  static async startDailyOperation(
    ...
    await SupabaseDatabaseService.updateUnit(unit.id, {
      isReceivable: true,
    ...
    try {
      ...
      if (user) {
        ...
        await SupabaseDatabaseService.updateUser(user.id, { points: newPoints });
  ...
  static async purchasePlan(
    ...
    const debitRes = await SupabaseDatabaseService.atomicDebitBalance(user.id, plan.priceUsdt);
    if (!debitRes.success) {
      return {
        success: false,
    ...
    const projects = await SupabaseDatabaseService.getProjects();
    ...
    const newUnit = await SupabaseDatabaseService.createUnit({
      userId: user.id,
      planId: plan.id,
      planCode: plan.code,
      planName: plan.name,
  ...
  static async upgradeUnit(
    ...
    const currentPlan = await SupabaseDatabaseService.getPlanByCode(unit.planCode);
    const targetPlan = await SupabaseDatabaseService.getPlanByCode(targetPlanCode);
    ...
    const debitRes = await SupabaseDatabaseService.atomicDebitBalance(user.id, upgradeCost);
    if (!debitRes.success) {
      return {
        success: false,
